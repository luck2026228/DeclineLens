...
| 想知道代码怎么组织的、参数在哪儿改 | [ARCHITECTURE.md](ARCHITECTURE.md) |
| 发现了安全问题 | [SECURITY.md](SECURITY.md) —— **别开公开 issue** |
| 想知道以后要做什么、为什么某个功能不做 | [ROADMAP.md](ROADMAP.md) |
| 看不懂 issue 上那些彩色标签 | 本文档 [第六节](#六标签是什么意思) |
...
报告是纯文本，只含 11 个诊断字段，不含任何隐私字段（[ARCHITECTURE.md](ARCHITECTURE.md#记录里存了哪-11-个字段) 列了完整清单）。
...
Issue 和 PR 上挂的那些彩色小牌子，分三类看。

**这是个什么事**

| 标签 | 意思 |
| --- | --- |
| `缺陷` | 装不上 / 不出球 / 结果不对 / 界面坏了 |
| `字典` | 新增或修正一条原因码的中文解释 —— **这个项目最欢迎的贡献** |
| `新功能` | 想加个新东西。先看 [ROADMAP.md](ROADMAP.md)，边界之内的才有戏 |
| `文档` | README / 说明书 / 代码注释写得不清楚 |
| `安全` | 隐私边界 / XSS / 权限。**别开公开 issue**，走 [SECURITY.md](SECURITY.md) |
| `构建 / CI` | 流水线、打包脚本、发版自动化 |
| `依赖` | CI 里那几个官方 Action 的版本跟进，Dependabot 自动打上 |
| `提问` | 使用上的疑问，不是缺陷。先看 [SUPPORT.md](SUPPORT.md) |
...
| --- | --- |
| `待确认` | 还没复现，需要更多信息 |
| `等你回复` | 挂着等你补充信息 —— 看到这个牌子，球在你那边 |
| `已确认` | 复现了，确实是个问题，排着队修 |
| `边界之外` | 明确不做。理由见[第五节](#五这个项目不想要什么) |
| `重复` | 已经有一条一模一样的了，去那条底下跟 |

**想搭把手的看这两个**

| 标签 | 意思 |
| --- | --- |
| `good first issue` | 改动小、影响面清楚，适合第一次提 PR |
| `help wanted` | 需要有人接手，我一时半会儿顾不上 |

这两个是全 GitHub 通用的名字，站内有专门的页面按它们搜人来帮忙，所以特意留了英文原名，没跟着翻译。
